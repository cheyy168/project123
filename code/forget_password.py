...
class Recovery:
    def __init__(self, user_data_file, backup_code_file):
        self._user_data_file = os.path.join("Database_txt", user_data_file)
        self._backup_code_file = os.path.join("Database_txt", backup_code_file)
        os.makedirs(os.path.dirname(self._backup_code_file), exist_ok=True)
    ...
    def _write_to_file(self, file_path, lines):
        """Safely writes lines to a file."""
        try:
    ...
    def save_backup_code(self, user_identifier, code):
        """Saves the backup code for a user."""
        try:
            with open(self._backup_code_file, "a") as file:
                file.write(f"{user_identifier},{code}\n")
        except Exception as e:
            print(f"Error saving backup code: {e}")
    ...
    def recover_password(self):
        ...
        while attempts < max_attempts:
            ...
            lines = self._read_file_lines(self._backup_code_file)
            remaining_lines = []
            code_verified = False
            ...
            for line in lines:
                ...
                if email_or_phone == stored_identifier and backup_code == stored_code:
                    ...
                    remaining_lines.append(line)  # Keep unused codes

            if code_verified:
                print("Backup code verified successfully!")
                self._write_to_file(self._backup_code_file, remaining_lines)  # Remove used code
                self.reset_password(email_or_phone)
                return
    ...
    def reset_password(self, user_identifier):
        """Allows the user to reset their password."""
        temp_file = self._user_data_file + ".tmp"
        password_updated = False
        ...
        for _ in range(3):  # Allow up to 3 attempts for a valid password
            ...
            lines = self._read_file_lines(self._user_data_file)
            updated_lines = []
            ...
            for line in lines:
                ...
                if user_identifier == email_or_phone:
                    salt = os.urandom(16)  # Generate a new salt
                    hashed_new_password = self.hash_password(new_password, salt)
                    updated_lines.append(f"{email_or_phone},{username},{salt.hex()},{hashed_new_password}\n")
                    password_updated = True
                else:
                    updated_lines.append(line)

            self._write_to_file(temp_file, updated_lines)

            if password_updated:
                os.replace(temp_file, self._user_data_file)
                print("Password reset successfully!")
                return
            else:
                print("User not found. Password not reset.")
